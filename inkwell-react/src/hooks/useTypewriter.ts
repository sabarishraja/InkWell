...
export interface TypewriterConfig {
  ...
  contentWidth?:            number;   // writable px width, default 552
...
export function useTypewriter(config: TypewriterConfig): TypewriterController {
  ...
  const updateCursorPosition = useCallback(() => {
    ...
    // Check right margin proximity
    const xRelToArea = x - (areaRect.left - parentRect.left);
    if (xRelToArea >= contentWidth - 50) {
      triggerMarginDing();
    }
  }, [contentWidth]);
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    // Printable character — let browser insert, sync cursor + sound
    audio.playKeyClack();
    pauseCursorBlink();
    setTimeout(updateCursorPosition, 10);
  }, [
